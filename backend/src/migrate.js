...
module.exports = async () => {
  // Drop and recreate tables that have schema mismatches
  const dropAndCreate = [
    // DISPATCH MODULE - fix dispatch_routes (needs route_number, estimated_start, estimated_end, created_by)
    'DROP TABLE IF EXISTS dispatch_stops',
    'DROP TABLE IF EXISTS dispatch_routes',
    `CREATE TABLE IF NOT EXISTS dispatch_routes (id INT AUTO_INCREMENT PRIMARY KEY, route_number VARCHAR(50), route_date DATE, driver_name VARCHAR(255), vehicle VARCHAR(100), status ENUM('planning','scheduled','in-progress','completed','cancelled') DEFAULT 'planning', total_stops INT DEFAULT 0, total_distance_miles DECIMAL(10,2), estimated_start DATETIME, estimated_end DATETIME, notes TEXT, created_by INT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)`,
    // dispatch_stops - fix stop_sequence (was stop_order)
    `CREATE TABLE IF NOT EXISTS dispatch_stops (id INT AUTO_INCREMENT PRIMARY KEY, route_id INT, stop_sequence INT, customer_id INT, customer_name VARCHAR(255), address TEXT, city VARCHAR(100), state VARCHAR(50), zip VARCHAR(20), sales_order_id INT, shipment_id INT, rack_id INT, estimated_arrival TIME, actual_arrival TIME, status ENUM('pending','en-route','arrived','delivered','failed','skipped') DEFAULT 'pending', signature_url VARCHAR(500), delivery_notes TEXT, photo_urls JSON, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`,
  ];

  // Tables that just need additional columns
  const alterations = [
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS work_center_capacity (id INT AUTO_INCREMENT PRIMARY KEY, work_center_id INT, day_of_week TINYINT, capacity_hours DECIMAL(5,2) DEFAULT 8.0, max_concurrent_jobs INT DEFAULT 1, shift_start TIME DEFAULT '07:00:00', shift_end TIME DEFAULT '15:30:00', is_available BOOLEAN DEFAULT TRUE, UNIQUE KEY unique_wc_day (work_center_id, day_of_week))`,
    ...
    `CREATE TABLE IF NOT EXISTS crm_activities (id INT AUTO_INCREMENT PRIMARY KEY, lead_id INT, customer_id INT, activity_type ENUM('call','email','meeting','note','task','follow-up','quote') DEFAULT 'note', subject VARCHAR(255), description TEXT, due_date DATETIME, scheduled_at DATETIME, completed_at DATETIME, outcome VARCHAR(255), status ENUM('planned','completed','cancelled','overdue') DEFAULT 'planned', assigned_to INT, created_by INT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`,
  ...
  // First run drop-and-recreate for tables with wrong schemas
  for (const sql of dropAndCreate) {
    try { await pool.query(sql); } catch(e) { console.log('Migration drop/create:', e.message.substring(0, 100)); }
  }

  // Then create all tables (IF NOT EXISTS will skip existing ones)
  for (const sql of tables) {
    try { await pool.query(sql); } catch(e) { console.log('Migration:', e.message.substring(0, 100)); }
  }

  // Then run alterations for existing tables that need new columns
