...
function VendorPayments() {
  ...
  const [form, setForm] = useState({ vendor_id: '', payment_date: new Date().toISOString().split('T')[0], amount: '', payment_method: 'check', check_number: '', bank_id: '', notes: '', applied_invoices: [] });

  useEffect(() => { fetchPayments(); fetchVendors(); fetchBanks(); }, []);

  const fetchPayments = async () => {
    try { const res = await api.get('/api/accounting/vendor-payments'); setPayments(Array.isArray(res.data) ? res.data : res.data.payments || []); } catch { setPayments([]); }
  };
  ...
  const handleSave = async () => {
    try { await api.post('/api/accounting/vendor-payments', form); toast.success('Payment recorded'); setShowNew(false); fetchPayments(); } catch (err) { toast.error(err.response?.data?.error || 'Failed'); }
  };
  ...
  return (
    <ModulePage {...accountingMenu}>
      ...
      <div className="erp-toolbar">
        <button className="erp-toolbar-btn" onClick={() => setShowNew(true)}><span className="text-green-600">+</span> Make Payment</button>
          ...
          <tbody>
            {payments.length === 0 ? <tr><td colSpan="7" className="text-center p-4 text-gray-500">No vendor payments</td></tr> : (payments || [])?.map(p => (
              <tr key={p.id}>
                ...
                <td><span className="px-2 py-0.5 rounded text-xs bg-orange-100">{p.payment_method}</span></td>
                <td>{p.check_number || '-'}</td>
                <td><span className={`erp-status erp-status-${(p.status || 'posted').toLowerCase()}`}>{p.status || 'posted'}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {showNew && (
        <div className="erp-modal-overlay" onClick={() => setShowNew(false)}>
          <div className="erp-modal" style={{ minWidth: '700px' }} onClick={e => e.stopPropagation()}>
            <div className="erp-modal-title"><span>Make Vendor Payment</span><button onClick={() => setShowNew(false)} className="text-white hover:text-gray-300">✕</button></div>
            ...
            <div className="erp-modal-footer">
              <button className="erp-btn erp-btn-primary" onClick={handleSave} disabled={!form.vendor_id || !form.amount}>Record Payment</button>
