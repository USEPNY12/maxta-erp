...
function Invoices() {
  ...
  const fetchInvoices = async () => {
    try { const res = await api.get('/api/sales/invoices', { params: { search, status: statusFilter } }); setInvoices(Array.isArray(res.data) ? res.data : res.data.invoices || []); } catch { setInvoices([]); }
  };

  const openDetail = async (inv) => {
    try { const res = await api.get(`/api/sales/invoices/${inv.id}`); setSelected(res.data); setActiveTab('Lines'); setShowDetail(true); } catch { setSelected(inv); setShowDetail(true); }
  };

  const handlePostInvoice = async () => {
    try {
      await api.put(`/api/sales/invoices/${selected.id}/post`);
